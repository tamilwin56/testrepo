...
# Temenos Transact — Agent_Broker Module Documentation
...
## Table of Contents
  ...
  - [Agent_Broker Module Overview](#agent_broker-module-overview)
    - [Sub-Module Summary](#sub-module-summary)
    - [How to Use This Guide](#how-to-use-this-guide)
  - [Chapter 1: Agent_Broker - AG](#chapter-1-agent_broker---ag)
    - [Features in Agent_Broker - AG](#features-in-agent_broker---ag)
    - [1.1  Activity Based Commissions](#11-activity-based-commissions)
    - [1.2  Agent Life Cycle Events](#12-agent-life-cycle-events)
    - [1.3  AgentFinancial Linkage](#13-agentfinancial-linkage)
    - [1.4  ClawBackOfCommission](#14-clawbackofcommission)
    - [1.5  ContextValueBasedCommission](#15-contextvaluebasedcommission)
    - [1.6  DeferringCommission](#16-deferringcommission)
    - [1.7  Migration of Agents from Legacy System](#17-migration-of-agents-from-legacy-system)
    - [1.8  Misc](#18-misc)
    - [1.9  MultipleAgents](#19-multipleagents)
    - [1.10  OnlineCommission](#110-onlinecommission)
    - [1.11  ScheduledCommission](#111-scheduledcommission)
    - [1.12  VolumeBasedCommission](#112-volumebasedcommission)
    - [1.13  Account Statements](#113-account-statements)
    - [1.14  Charges and Commissions](#114-charges-and-commissions)
    - [1.15  Hybrid Pooling](#115-hybrid-pooling)
    - [1.16  Misc](#116-misc)
    - [1.17  Charges](#117-charges)
    - [1.18  SubAccounts](#118-subaccounts)
    - [1.19  Limit Balance](#119-limit-balance)
    - [1.20  Misc](#120-misc)
    - [1.21  Chargeoff](#121-chargeoff)
    - [1.22  Charges](#122-charges)
    - [1.23  LendingRule78](#123-lendingrule78)
    - [1.24  Loan Commitment](#124-loan-commitment)
    - [1.25  Migration of LendingArrangements](#125-migration-of-lendingarrangements)
    - [1.26  Payment Holiday](#126-payment-holiday)
    - [1.27  Scheduling Payments](#127-scheduling-payments)
    - [1.28  Weighted Average Rate](#128-weighted-average-rate)
    - [1.29  Misc](#129-misc)
  ...
  - [Appendix B: Consolidated Field Reference](#appendix-b-consolidated-field-reference)
    - [Agent_Broker - AG (AG)](#agent_broker---ag-ag)

---


## Agent_Broker Module Overview


This document provides comprehensive documentation for the **Agent_Broker** module of Temenos Transact. It covers **1 sub-modules** with a total of **29 feature areas**.
...
|---|-----------|------|----------|-------------|
| 1 | **Agent_Broker - AG** | `AG` | 29 | Agent_Broker - AG module of Temenos Transact |
...
## Chapter 1: Agent_Broker - AG


Agent_Broker - AG module of Temenos Transact


### Features in Agent_Broker - AG


| # | Feature | Sections |
|---|---------|----------|
| 1.1 | Activity Based Commissions | Intro, Tasks, Outpu |
| 1.2 | Agent Life Cycle Events | Intro, Tasks, Outpu |
| 1.3 | AgentFinancial Linkage | Confi, Worki, Tasks, Outpu |
| 1.4 | ClawBackOfCommission | Intro, Tasks, Outpu |
| 1.5 | ContextValueBasedCommission | Intro, Tasks, Outpu |
| 1.6 | DeferringCommission | Intro, Tasks, Outpu |
| 1.7 | Migration of Agents from Legacy System | Intro, Confi, Worki, Tasks, Outpu |
| 1.8 | Misc | Intro |
| 1.9 | MultipleAgents | Intro, Tasks, Outpu |
| 1.10 | OnlineCommission | Intro, Tasks, Outpu |
| 1.11 | ScheduledCommission | Intro, Tasks, Outpu |
| 1.12 | VolumeBasedCommission | Intro, Tasks, Outpu |
| 1.13 | Account Statements | Intro |
| 1.14 | Charges and Commissions | Worki |
| 1.15 | Hybrid Pooling | Worki |
| 1.16 | Misc | Intro |
| 1.17 | Charges | Worki |
| 1.18 | SubAccounts | Confi |
| 1.19 | Limit Balance | Intro |
| 1.20 | Misc | Intro |
| 1.21 | Chargeoff | Worki |
| 1.22 | Charges | Worki |
| 1.23 | LendingRule78 | Intro |
| 1.24 | Loan Commitment | Confi |
| 1.25 | Migration of LendingArrangements | Worki |
| 1.26 | Payment Holiday | Intro, Confi, Worki |
| 1.27 | Scheduling Payments | Confi, Worki |
| 1.28 | Weighted Average Rate | Intro |
| 1.29 | Misc | Intro |


### 1.1  Activity Based Commissions


> **📇 Quick Reference Card**
> 
> **Purpose:** *Agents avail commission for the services they provide to the bank customers. Agents help customers in availing the banking products. This payment can be a fixed or calculated amount based on the deal amount and it is paid to the agent as per the agreement with the bank. The amount of commission is r...*
> 
> **Key Fields:** *Arrangement*
> 
...
Agents avail commission for the services they provide to the bank customers. Agents help customers in availing the banking products. This payment can be a fixed or calculated amount based on the deal amount and it is paid to the agent as per the agreement with the bank. The amount of commission is related to an activity. Authorising this activity triggers the commission payment to the agent.

An option is available to restrict commission payment until full funding is executed (full disbursement of loan or funding of deposits).

Additionally, this feature can also be used to make commission payment as a manual or automated process.

- Manual - The user manually triggers commission payment based on a specific activity and it is paid online.
- Automatic - The system can pay out the commission on a regular frequency using a batch service.
...
Commissions for Agents can be negotiated on at a financial arrangement or Financial Product. This can be triggered through the Agent Update activity for agent commission.
...
|---|---|
|  | Authorised . |
| Find Account Arrangements | Enter a personal loan arrangement ID in the Arrangement field and then click the FIND button. Click the Overview icon of a corresponding record. |
| Arrangement Overview (Accounts) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to the Update Activity for Agent Commission activity. |
| Arrangement Activity | Enter values in the following fields: Agent Id Agent Arr Id Agent Role Click the Validate icon. Click the Agent Commission section and then enter values in the following fields: Commission Operand Rate Click the Validate icon to check for errors and overrides. Click the Commit icon. |


#### 📊 Outputs

There are no Outputs available for Activity Based Commissions features.

---


### 1.2  Agent Life Cycle Events


> **📇 Quick Reference Card**
> 
> **Purpose:** *In the life cycle of an agent arrangement, the bank creates the arrangement and initiates changes for the same. The following are some of the life cycle events:*
> 
> **Key Fields:** *Arrangement*, *GB
...
In the life cycle of an agent arrangement, the bank creates the arrangement and initiates changes for the same. The following are some of the life cycle events:

- Agent arrangement creation
- Agent arrangement amendment
- Agent status maintenance
- Changing the product
- Re-negotiate the product
...
- Link Agent Arrangement with Financial Products
- Create New Agent Arrangement

In the life cycle of an agent arrangement, the bank creates an arrangement and initiate changes for the same. Agent arrangement governs the customer or bank initiated events during the course of the agent’s engagement with the bank. These events are achieved by initiating relevant activities in the system.


##### Workflow

The section allows the user to perform the below tasks:

| SCREENS | WORKFLOW |
|---|---|
|  | Product Catalog . |
| Product Catalog | Click the 3rd Party Distributor option to expand. Click the New Arrangement icon corresponding to the Affiliated Agent product. |
| New Arrangement | Enter values for the following fields: Customer Currency Click the Validate icon to check for errors and overrides. Click the Settlement Instructions tab to expand. Select the Basic tab. In Charges Due Settlement section, enter a value in the Settlement A/c field. In Payout Commission section, enter a value in the Settlement A/c field. Click the Validate icon to check for errors and overrides. Click the Commit icon to submit the record. |

| SCREENS | WORKFLOW |
|---|---|
|  | Authorised . |
| Find Lending Arrangement | Enter a value in the Arrangement field and then click the FIND button. Click the Overview icon corresponding to a record. |
| Arrangement Overview (Lending) | Click the New Activity icon. |
| New User Activities | Click the Do Activity Today icon corresponding to Update activity for Account option. |
| Arrangement Activity | Select the Arrangement Activity tab. Enter values in the following fields: GB Account Title Alternate ID Click the Validate icon to check for errors and overrides. Click the Commit icon to submit the record. |

| SCREENS | WORKFLOW |
|---|---|
|  | Authorised |
| Find Lending Arrangements | Enter a value in the Arrangement field and then click the FIND button. Click the Overview icon corresponding to a record. |
| Arrangement Overview (Lending) | Click the New Activity icon. |
| New User Activities | Click the Do Activity Today icon corresponding to Deactivate Agent Arrangement option. |
| Arrangement Activity | Select the Arrangement Activity tab. Enter a value in the GB Narrative field. Click the Validate icon to check for errors and overrides. Click the Commit icon to submit the record. |
...
| Find Lending Arrangements | Enter a value in the Arrangement field and then click the FIND button. Click the Overview icon corresponding to a record. |
| Arrangement Overview (Lending) | Click the New Activity icon. |
| New User Activities | Click the Do Activity Today icon corresponding to the Change Product activity option. |
| Arrangement Activity | Select the Product from the drop-down list. Click the Validate icon to check for errors and overrides. Click the Commit icon to submit the record. |
...
| Find Lending Arrangements | Enter a value in the Arrangement field and then click the FIND button. Click the Overview icon corresponding to a record. |
| Arrangement Overview (Lending) | Click the New Activity icon. |
| New User Activities | Click the Do Activity Today icon corresponding to Change Agent/Update Commission option. |
| Arrangement Activity | Enter values in the following fields: Agent Id Agent Arr Id Click the Validate icon to check for errors and overrides. Click the Commit icon to submit the record. |

| SCREENS | WORKFLOW |
|---|---|
|  | Single Customer View . |
| Customer Details | Select the Agent Arrangements tab. |
| Current Arrangement | Click the Pay Out Commission icon. |
| Agent Commission - Payout | Enter values in the following fields: Arrangement Id Debit Currency Credit Account Debit Amount Click the Validate icon to check for errors and overrides. Click the Commit icon to submit the record. |
...
| Find Lending Arrangements | Enter a value in the Arrangement field and then click the FIND button. Click the Overview icon corresponding to a record. |
| Arrangement Overview (Lending) | Click the New Activity icon. |
| New User Activities | Click the Do Activity Today icon corresponding to Close Activity for Arrangement option. |
| Arrangement Activity | Enter values in the following fields: GB Closure Notes Closure Reason Click the Validate icon to check for errors and overrides. Click the Commit icon to submit the record. |


#### 📊 Outputs

This topic helps the user to view the enquiries and reports of Agent Life Cycle Events feature.


##### Enquiries and Reports

The navigation and uses of the enquiry is given in the below drop-down:

Bills

The details of all Agent bills are displayed in this enquiry. Options are also available to view the commission record.
...
### 1.3  AgentFinancial Linkage


> **📇 Quick Reference Card**
> 
> **Key Fields:** *Agent Arr Id*, *Agent Id*, *Context Name*, *Context Value*, *Currency*, *Customer*, *Effective Date*, *Policy.Term*
> 
...
The Agency Commission Property is attached to the underlying product for which a financial arrangement can be created.

At the time of creating a financial arrangement for the product, the agent details are included by way of updating the customer and arrangement number pertaining to the Agent product.


#### 🔧 Working With


##### Agent Arrangement in Financial Product

The agent information is keyed at the time of arrangement creation of the financial product. The Product Commission conditions in Agent Arrangement are updated to the Agent Commission condition in this arrangement.


##### Agent Arrangement in External Product

Agents can be assigned to arrangements created for external products. External products also include External Property Class. To know more refer to External Products userguide.

For example, Home Owner Insurance product group under Insurance – External Product (XINSURANCE) product line has built-in Property Classes. This product group has a user defined property class as well namely, XINSURANCE.

The users create customised Property Classes using AA.CLASS.DEFINITION . Such Property Classes are used to create customised Product Lines. For example, the user can customise the Policy Term, Frequency.

The system automatically creates a designer application based on the records created in AA.CLASS.DEFINITION . It is then used in building the Product.

The Policy Term, Frequency, Premium Amount and Installment attributes are now in the user-defined Property Class.

The AA.SOURCE.CALC.TYPE record is a way to define the initial mapping from an AA.PROPERTY in this case XINSURANCE and a particular Policy.Term field.

External applications (for example Agents) pass information using a feed. A user definable value prefixed by U is a way information is passed from one arrangement via a Context Name and Context Value.

AA.ARRANGEMENT.ACTIVITY has the Context Name and Context Value fields to consume the information and pass the amortisation term to the agency arrangement. For example, U-AMORTIZATION.TERM is a user defined record defined as a period which is used to map a value to the agent record.

In this table the AA.CONTEXT.TYPE and AA.SOURCE.CALC.TYPE is linked together. This record can be defined at the agent PRODUCT.COMMISION record for the trigger activity linked to the XINSURANCE product. This configuration is used as a linkage in other tables.

Amortisation of Commission can be setup using the Accounting Property Condition as a specific period like 1M, 1Y, etc can be a based on arrangement information like maturity, renewal date or next schedule date. For agent commission, a user defined Context Type can be fed as amortisation period. The context type is period based context type and attribute based.

In the external arrangement, the premium paid period is defined, say one time or periodical. If it’s a onetime payment, then the attribute is updated with policy term. In case of periodical premium payment, it is updated with the frequency of premium payment.

The system passes the value in this attribute to the agent event record, whenever a commission has to be calculated. When the commission is set to amortise, the value passed in the context is updated as the amortisation period.

> **⚠️ Note:** Fetching the amortisation term from the external arrangement to calculate commission is applicable only for agent commission.
...
- Link Agent Arrangement with Financial Products
- Link Agent Arrangement with Financial Product

When creating a financial arrangement for the product, the agent details are included by way of updating the customer and arrangement number pertaining to the Agent product.
...
|---|---|
|  | Product Catalog . |
| Product Catalogue | Click the New Arrangement icon of a corresponding to Current Account. |
| New Arrangement | Enter values in the following fields: Customer Currency Agent Id Agent Arr Id Effective Date Click the Validate icon to check for errors and overrides. Click the Commit icon. |


#### 📊 Outputs

There are no Outputs available for Agent and Financial Arrangement Linkage feature.

---


### 1.4  ClawBackOfCommission


> **📇 Quick Reference Card**
> 
> **Purpose:** *The user can claw back (also known as draw back) the commission paid to an agent, where the customer forecloses the loan or redeems the deposit early. However, this is done only when the commission is set to Amortise in the Accounting Property Condition.*
> 
> **Key Fields:** *Arrangement*, *Drawback Type*
> 
> **Sections:** 📖 Introduction | 📋 Tasks | 📊 Outputs
...
The user can claw back (also known as draw back) the commission paid to an agent, where the customer forecloses the loan or redeems the deposit early. However, this is done only when the commission is set to Amortise in the Accounting Property Condition.

The business activity, redeem in case of deposits and prepay/settle in case of loans, triggers the claw back of commission paid to the related agent.
...
The user can claw-back (also known as draw back) the commission paid to an agent, when the customer forecloses the loan or redeems the deposit early. Claw-back of commission is applicable only for Online Commission. The Drawback Type field allows one of the following values:

- None – No claw-back is required and the remaining unamortised amount is booked to PL
- Full – Claw-back is done for the original commission amount paid to the agent
- Partial – Claw-back is done for the unamortised commission amount
...
|---|---|
|  | Authorised . |
| Find Agent Arrangements | Enter an account arrangement ID in the Arrangement field and then click the FIND button. Click the Overview icon of a corresponding record. |
| Arrangement Overview (Agents ) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to Update Product Commission activity. |
| Arrangement Activity | Click the Validate icon. Click the Product Commission section and then enter a value in the Drawback Type field. Click the Validate icon to check for errors and overrides. Click the Commit icon. |


#### 📊 Outputs

There are no Outputs available for Claw Back of Commission feature.

---


### 1.5  ContextValueBasedCommission


> **📇 Quick Reference Card**
> 
> **Purpose:** *The user can calculate commission to agents based on external context values such as average outstanding balance, principal increase, etc. on the customer’s financial arrangements.*
> 
> **Key Fields:** *Amount*, *Arrangement*, *Calc Type*, *Currency*, *Default Values*, *Rate*, *Tier Group*, *Tier Type* ... +1 more
> 
> **Sections:** 📖 Introduction | 📋 Tasks | 📊 Outputs
...
The user can calculate commission to agents based on external context values such as average outstanding balance, principal increase, etc. on the customer’s financial arrangements.

AG module supports definition of agent commission as a fixed amount or percentage rate, based on loan amount of the financial arrangement.

The agent commission is defined based on a percentage rate on external context values that are derived from the financial arrangement.

Agent commission can be defined based on the term of the financial arrangement, which becomes the external context to the agent arrangement for the purpose of commission calculation.

Agent commission can be calculated based on different balances in the financial arrangement such as:

- Original loan amount
- Average outstanding balance
- Principal balance on renewal
- Principal increase
...
| Find Agent Arrangements | Enter an account arrangement ID in the Arrangement field and then click the FIND button. Click the Overview icon of a corresponding record. |
| Arrangement Overview (Agents) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to Change Personal Loan Commission activity. |
| Arrangement Activity | Click the Validate icon. Click the Personal Loan Commission section and then click the Default Values (Calculation) icon. Enter values in the following fields: Type Currency Amount Tier Group Tier Type Calc Type Rate Click the Validate icon to check for errors and overrides. Click the Commit icon. |


#### 📊 Outputs

There are no Outputs available for Context Value-Based Commission feature.

---


### 1.6  DeferringCommission


> **📇 Quick Reference Card**
> 
> **Purpose:** *The user can defer or negotiate the commission paid to an agent only after 15 or 30 days from the date of disbursement of the full loan amount.*
> 
> **Key Fields:** *Arrangement*, *Defer Days*
> 
> **Sections:** 📖 Introduction | 📋 Tasks | 📊 Outputs
...
The user can defer or negotiate the commission paid to an agent only after 15 or 30 days from the date of disbursement of the full loan amount.
...
The user can defer or negotiate the commission paid to an agent only after 15 or 30 days from the date of disbursement of the full loan amount. The Defer Days field is available in the Product Commission Product Condition. This field is part of online commission multi-value set and holds the value as number of days from the online activity or financial event after which the commission is calculated and paid to the agent.
...
| Arrangement Overview (Agents) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to Update Product Commission activity. |
| Arrangement Activity | Click the Validate icon. Click the Product Commission section and then enter a value in the Defer Days field. Click the Validate icon to check for errors and overrides. Click the Commit icon. |


#### 📊 Outputs

There are no Outputs available for Deferring Commission feature.

---


### 1.7  Migration of Agents from Legacy System


> **📇 Quick Reference Card**
> 
> **Purpose:** *Existing contracts are most likely to be present when the Temenos Transact system replaces the existing legacy system. Therefore, these contracts need to be transferred to the new Temenos Transact system, which is referred to as the Takeover process. Similar to the Takeover process of contracts (tha...*
> 
> **Key Fields:** *Bill Date*, *Bill Reference*, *Bill Type*, *New Property Amount*, *Original Property Amount*, *Payment Date*, *Payment Method*, *Payment Type*
> 
...
Existing contracts are most likely to be present when the Temenos Transact system replaces the existing legacy system. Therefore, these contracts need to be transferred to the new Temenos Transact system, which is referred to as the Takeover process. Similar to the Takeover process of contracts (that is, Loans, Deposits and Accounts) the agent contracts and details are also taken-over from the legacy system.

As each new implementation is different, a general method is adopted when planning and executing the takeover of existing agents into the retail Agent product line. Read the Working with section for more information.


#### ⚙️ Configuration

Takeover of live contracts uses the Balance Maintenance Property Class. For more information on configuration, read the Balance Maintenance Property Class.


#### 🔧 Working With

The following section describes the takeover process in detail.
...
- Analyse the existing products and create equivalent products using the Agency (AG) module.
- Determine requirements for historic data in Temenos Transact.
- Prepare an extract of legacy agent contract data and balances.
- Create new arrangements from the legacy contract data.
- Create the outstanding legacy bills.
- Update the accrual balances.


##### Takeover of Agent Arrangement

The agent arrangement is taken over from the legacy system. Based on the requirements agent arrangements are taken over to the new product created in Temenos Transact. The following data forms part of the arrangement taken over:
...
Thus the Agent Arrangement is taken over from the legacy system to Temenos Transact.


##### Capture Bills

Any bills pending to be settled in the legacy system has to be taken over and created in Temenos Transact These outstanding bills can be one of the following:

- Due Type - To be paid by the customer or
- Pay Type - To be paid to customer.

The Capture Bill Activity can be used to capture a Due bill or Pay bill that is outstanding in the legacy system and needs to be migrated to Temenos Transact.

Consider the NEWAGENTFEE that is due:

- The Bill Reference is updated as NEW – The bill number is automatically generated by the system when the Activity is committed
- The Bill Date and Payment Date are the original bill generation date in the legacy system. This date should be after the Original Contract Date of arrangement activity.
- The Bill Type and Payment Type updated as per the product setup for the NEWAGENTFEE.
- Original Property Amount is the original bill amount
- New Property Amount is the outstanding amount pending to be settled.
- If the bill is aged, the same can be updated as delinquent amount.
- The Payment Method is Due indicating that bill has to be paid by the customer to the bank.

Consider the DEPCOMMISSION which is a pending deposit commission to be paid to the Agent.

- The Bill Reference is updated as NEW – The bill number is automatically generated by the system when the Activity is committed
- The Bill Date and Payment Date are the original bill generation date in the legacy system. This date should be after the Original Contract Date of arrangement activity.
- The Bill Type and Payment Type updated as per the product setup for the DEPCOMMISSION. Any commission is of the Bill Type Commission and Payment Type Charge.
- Original Property Amount is the original bill amount
- New Property Amount is the outstanding amount pending to be settled.
- The Payment Method is Pay indicating that bill has to be paid by the customer to the bank.


##### Capture Balances

For a commission (Charge Property) that is set to accrue, the accruals can be taken over using Capture Balance Activity. In this case, MLPERIODICCOM Adjust Property is accrued and their accruals from legacy system is captured in the Adjust Balance Type ACCMLPERIODICCOM. The New Balance Amount or Net Adjustment Amount is 56.67 indicates the accruals are positive and is payable to the customer.

> **⚠️ Note:** For any accruals that are due from the customer, the amount captured is negative. For any accruals that are to be paid to the customer, the amount captured is positive.


##### Arrangement Balances post Takeover

The outstanding bill and balances of the agent are available in T24 Transact and can be viewed in the EB.CONTRACT.BALANCES Enquiry.


#### 📋 Tasks

There are no tasks available for Migration of Agents from Legacy System feature.


#### 📊 Outputs

There are no Outputs available for Migration of Agents from Legacy System feature.
...
> **📇 Quick Reference Card**
> 
> **Purpose:** *For financial marketplaces to function effectively, investing participants link up with those who require funds, which is not always direct. Financial intermediaries smoothen this flow of funds to needy participants by linking them and providing essential services.*
> 
> **Key Fields:** *Accrual Id*, *Accrue or*, *Amortise*, *Amount*, *Arrangement*, *Commission Nature*, *Event Activity*, *Event Function* ... +12 more
> 
...
- In the AA.PROPERTY application, the user has to define the Property Type field as Commission.
...
AGENT-TRIGGER- *MONTHLY

Where, in the above Activity specifies Charge Property and frequencies are defined in AA.AGENT.COMMISSION.FREQUENCY . For example, AGENT-TRIGGER-ACCHARGE*MONTHLY, where ACCHARGE is a Charge Property.

Online processing is done using Update Activity Charges (AGENT-TRIGGER- ) while Scheduled processing is done by appending the frequency after the Charge Property.

The ACTIVITY.CHARGES-CALCULATE.COMM action calculates and processes the commission on the agent arrangement.

The -HANDOFF-AGENT.COMMISSION* Activity Class is used for financial arrangements that pay scheduled commissions to its agents:
...
> **⚠️ Note:** This is a system activity.
...
AA.AGENT.COMMISSION.FREQUENCY and the frequencies are given below.
...
- For online commissions, the system captures the Trigger Activity for Accharge (AGENT-TRIGGER- ). For example, AGENT-TRIGGER-ACCHARGE.
- For scheduled commissions, Trigger Activity for Accharge (AGENT-TRIGGER- * ) is used. (For example, AGENT-TRIGGER-ACCHARGE*MONTHLY)
...
> **⚠️ Note:** To process the failed transactions again, the user can use verify function on the AA.AGENT.EVENT record.

The commission calculated from each financial arrangement is stored in AA.AGENT.COMMISSION.DETAILS . This information is useful while sending account statements to agents and showing how the scheduled commission amount was derived.

- The record Id in this application is the ID of AA.BILL.D E TAILS linked to the agent arrangement.
- The Arrangement field is the financial arrangement that triggered the commission payment.
...
> **⚠️ Note:** The EB.ACCRUAL ID gets stored only when the commission is amortised on the agent arrangement.
...
> **⚠️ Note:** The value in Accrual Id field is used when processing claw back of commission paid.
...
|---|---|---|
| 1 | PRODUCT.COMMISSION | This Property Class is used to configure financial Product Line / Product Group / Products those are eligible for commission calculation. It provides the option to calculate commission (Online/Scheduled) as either percentage or fixed amount with an option to claw-back commission when generated business is closed ahead of the agreed term |
| 2 | AGENT.COMMISSION | This Property Class allows to setup linkage between financial product and corresponding Agent arrangement. Blank Agent Commission is attached to financial product in turn referring to Product Commission condition setup at Agent. |
...
> **📇 Quick Reference Card**
> 
> **Purpose:** *Multiple brokers, agents and intermediaries can be linked to one financial arrangement. Based on the roles of the agents, there can be one or multiple agents related to a financial arrangement.*
> 
> **Key Fields:** *Agent Arrangement ID*, *Agent Customer ID*, *Agent ID*, *Agent role*, *Arrangement*, *Currency*, *Customer ID*, *Effective Date* ... +5 more
> 
> **Sections:** 📖 Introduction | 📋 Tasks | 📊 Outputs
...
Multiple brokers, agents and intermediaries can be linked to one financial arrangement. Based on the roles of the agents, there can be one or multiple agents related to a financial arrangement.

In case of multiple agents, commission can be paid for various activities and agents.

Roles like Lead Agent, Referencing broker, etc. can be assigned to variousagents.
...
Whenever a new financial Lending, Deposit or Account Arrangement is created, It is essential to provide the following details:

- Customer ID
- Product
- Currency
- Effective Date
- Variation and Pricing details
- Agent ID
- Agent Arrangement ID
- Agent role

A financial arrangement can have more than one Agent or Intermediaries working on it. Thus, Temenos Transact allows you to link Multiple brokers, agents and Intermediaries to the same financial contract through the following multi-value fields:

- Agent Customer ID
- Agent Arrangement ID
- Role of the Agent

When a new financial arrangement is created by linking an ‘Inactive’ Agent Arrangement, an override message appears indicating the agent is inactive.

User can create any accounts, deposit or lending product, where the Agent Customer ID, Agent Arrangement ID and Agent Role have to be entered and linked with financial arrangement.


##### Workflow

This section allows the user to perform the below task:

To create a new Arrangement, follow the below steps:

1. Personal Loans .
2. Click the New Arrangement icon corresponding to the desired product.
3. Enter values in the following mandatory fields: Customer ID Product Currency Effective Date Variation and Pricing details Agent ID Agent Arrangement ID Agent role
4. Click the Add row icon to enter another agent details.
5. Click the Validate icon to check for errors and overrides.
6. Click the Commit icon to submit the record for authorisation.


#### 📊 Outputs

This topic helps the user to view the enquiries and reports of Multiple Agents feature.


##### Enquiries and Reports

The navigation and uses of the enquiry is given in the drop-down:

Find Loan

Personal loan Overview is designed to provide 360-degree view of an account. It is a composite screen with all account related facilities and enquiries, at a single place. To Overview a loan perform the following steps:

1. Click the Find button after entering values in one or more of the following fields:

1. Select the required loan from the search results and click the Overview icon corresponding to it.
...
### 1.10  OnlineCommission


> **📇 Quick Reference Card**
> 
> **Purpose:** *Using this feature, the user can tag commission payment to a specific activity (that is paid only when the activity is completed). For instance, if the commission payment is tagged to full disbursement of loan, then the payment is made only when the full loan amount is disbursed to the user.*
> 
> **Key Fields:** *Amount*, *Arrangement*, *Calc Type*, *Commission Type*, *Currency*, *Defer Day*, *Drawback Type*, *Financial Activity* ... +8 more
> 
> **Sections:** 📖 Introduction | 📋 Tasks | 📊 Outputs
...
Using this feature, the user can tag commission payment to a specific activity (that is paid only when the activity is completed). For instance, if the commission payment is tagged to full disbursement of loan, then the payment is made only when the full loan amount is disbursed to the user.
...
Online Commission enables the user to define the Product Lines, Product Groups and Products that qualify for a commission. This option links all other AA financial Product Lines to enable any arrangement to be included in an agent’s compensation.

This option at the product level allow the users to configure the following:

- Financial Product Lines/Product Groups/Products that are eligible for commission calculation.
- To define if commission has to be paid immediately (online).
- To define if commissions can be on a scheduled basis.
- To specify if claw-back of commission has to be done (Applicable only for Online Commission).
- To allow deferring of commission calculation.
- To allow choosing base date for a scheduled agent commission.

Based on the way the commission is paid to an agent, they are classified into the following types:
...
| Arrangement Overview (Agents ) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to Update Product Commission activity. |
| Arrangement Activity | Click the Validate icon. Click the Product Commission section and then enter values in the following fields: Product Line Product Group Product Financial Activity Online Charge Drawback Type Defer Day s Commission Type Click the Validate icon to check for errors and overrides. Click the Commit icon. |
...
| Find Agent Arrangements | Enter an account arrangement ID in the Arrangement field and then click the FIND button. Click the Overview icon of a corresponding record. |
| Arrangement Overview (Agents ) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to Update Deposit Commission activity. |
| Arrangement Activity | Click the Validate icon. Click the Deposit Commission section and then enter values in the following fields: Type Currency Tier Groups Tier Type Calc Type Rate Amount Click the Validate icon to check for errors and overrides. Click the Commit icon. |
...
| Find Agent Arrangements | Enter an account arrangement ID in the Arrangement field and then click the FIND button. Click the Overview icon of a corresponding record. |
| Arrangement Overview (Agents ) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to Update AVI Commission activity. |
| Arrangement Activity | Click the Validate icon. Click the Insurance – AVIVA commission section and then enter values in the following fields: Type Currency Tier Groups Tier Type Calc Type Rate Amount Click the Validate icon to check for errors and overrides. Click the Commit icon. |


#### 📊 Outputs

There are no Outputs available for Online Commission feature.

---


### 1.11  ScheduledCommission


> **📇 Quick Reference Card**
> 
> **Purpose:** *Using this feature, the user can pay the commission to agents on a pre-defined frequency.*
> 
> **Key Fields:** *Arrangement*, *Credit Account*, *Debit Account Number*, *Linked Int Property*, *Payment Amount*, *Product*, *Product Group*, *Product Line* ... +3 more
> 
> **Sections:** 📖 Introduction | 📋 Tasks | 📊 Outputs
...
Using this feature, the user can pay the commission to agents on a pre-defined frequency.

If the commission payment frequency is set to monthly, then, the commission is paid to the agent every month (that is, when the customer pays part of the loan as per schedule).
...
- Financial Product Lines/Product Groups/Products those are eligible for commission calculation.
- To define if commissions can be on a scheduled basis.
- To specify if claw-back of commission has to be done (Applicable only for Online Commission).
- To allow deferring of commission calculation.
- To allow choosing base date for a scheduled agent commission.
...
| Arrangement Overview (Agents ) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to Update Product Commission activity. |
| Arrangement Activity | Click the Validate icon. Click the Product Commission section to expand. Click the Product Commission - Schedule section and then enter values in the following fields: Product Line Product Group Product Schedule Name Schedule Charge Linked Int Property Schedule Frequency Click the Validate icon to check for errors and overrides. Click the Commit icon. |

This option allows user to initiate the agent commission pay-in via payment order.
...
This option allows user to initiate the agent commission payout via payment order.

1. Agent Commission Payout(PO) .
2. Enter values in the following fields: Debit Account Number Credit Account Payment Amount
3. Click the Validate icon to check for errors and overrides.
4. Click the Commit icon.
...
There are no Outputs available for Scheduled Commission feature.

---


### 1.12  VolumeBasedCommission


> **📇 Quick Reference Card**
> 
> **Purpose:** *Commission can be paid to agents based on the volume of business done by the agent. So, when an agent sources a business to the bank, the details are stored (which are periodically checked using a Batch service) and the commission is paid accordingly.*
> 
> **Key Fields:** *Arrangement*, *Calc Type*, *Currency*, *Customer*, *Effective Date*, *Fixed Amount*, *GB Narrative*, *Margin Rate* ... +8 more
> 
> **Sections:** 📖 Introduction | 📋 Tasks | 📊 Outputs
...
Commission can be paid to agents based on the volume of business done by the agent. So, when an agent sources a business to the bank, the details are stored (which are periodically checked using a Batch service) and the commission is paid accordingly.
...
Commission can be paid to agents based on the volume of business done by the agent.
...
|---|---|
|  | Product Catalog . |
| Product Catalogue | Click the New Arrangement icon of a corresponding to Affiliated Agent product. |
| New Arrangement | Enter values in the following fields: Customer Currency Effective Date Click the Validate icon. Click the Volume Based Commission option and then click the Default Values (Calculation) icon. Enter values in the following fields: Type Currency Fixed Amount Tier Group Tier Type Calc Type Rate Click the Validate icon to check for errors and overrides. Click the Commit icon. |

| SCREENS | WORKFLOW |
|---|---|
|  | Authorised . |
| Find Agent Arrangements | Enter an account arrangement ID in the Arrangement field and then click the FIND button. Click the Overview icon of a corresponding record. |
| Arrangement Overview (Agent) | Click the New Activity option. |
| New Activities | Click the Do Activity Today icon corresponding to the Update Volume Based Commission activity. |
| Arrangement Activity | Enter a value in the GB Narrative field. Click the Validate icon. Click the Volume Based Commission option and then click the Default Values (Calculation) icon. Enter values in the following fields: Rate Period Negative Rate Margin Type Operand Margin Rate Click the Validate icon to check for errors and overrides. Click the Commit icon. |


#### 📊 Outputs

There are no Outputs available for Volume Based Commission feature.

---


### 1.13  Account Statements
...
### 1.25  Migration of LendingArrangements
...
| `Alternate Id Type` | `Amortisation Term` | `Amortise` |
| `Amount` | `Application Method` | `Arrangement` |
| `Attribute Type` | `Attribute Value` | `Back-to-Back FX` |
| `Back-to-back FX` | `Balance Treatment` | `Base Ccy Product` |
| `Bill Produced` | `Bill Date` | `Bill Produced` |
| `Bill Reference` | `Bill Status` | `Bill Type` |
...
| `Charge off Percentage` | `Combine Bill At Parent` | `Commission Nature` |
| `Commission Type` | `Commitment Drawdown` | `Commitment Reversal` |
| `Comparison Type` | `Context Name` | `Context Value` |
| `Context name` | `Cooling Date Adj` | `Cooling Period` |
| `Create Retry Trigger` | `Credit Account` | `Currency` |
| `Currency Market` | `Custom Priority Rank` | `Customer` |
| `Customer ID` | `Date Convention` | `Debit Account Number` |
| `Def Account` | `Def Prev Settle` | `Default Values` |
| `Defer Date` | `Defer Day` | `Defer Days` |
| `Defer Period` | `Drawback Type` | `Due Frequency` |
| `Due Rule` | `Due Type` | `EIR` |
| `Effective Date` | `Emit Business Event` | `End Date` |
| `Event Activity` | `Event Function` | `Event Status` |
| `Event Type` | `Expiry date` | `Extend Cycle` |
| `External Availability` | `External Posting` | `Field Name.1.1` |
| `Field Name.1.2` | `Field Name.1.3` | `Field Name.1.4` |
| `Field Name.1.5` | `Field Name.1.6` | `Field Value.1.1` |
| `Field Value.1.2` | `Field Value.1.3` | `Field Value.1.4` |
| `Field Value.1.5` | `Field Value.1.6` | `Finalise Bills` |
| `Finalise Bills` | `Financial Activity` | `Financial Date` |
| `Fixed Amount` | `Frequency` | `Full Chargeoff` |
| `Full Commitment Activity` | `Fwd Acct Mode` | `GB Narrative` |
...
| `Legacy War Rate` | `Link AC Number` | `Link Account` |
| `Linked Ac Number` | `Linked Account Number` | `Linked Int Property` |
| `Loc Cap Chk Rtn` | `Loc Processor` | `Loc post process` |
| `Loc pre process` | `Loc prioritise` | `Loc processor` |
| `Mandatory` | `Margin Rate` | `Margin Type` |
| `Master Arrangement` | `Master Live Date` | `Max Percentage` |
| `Maximum Amt.1` | `Maximum Term Cap` | `Method` |
| `Min. Tfr Cr 1` | `Min. Tfr Dr 1’` | `Minimum Amt.1` |
| `Minimum Payment Type` | `Modify/Cancel Payment Holidays` | `Negative Rate` |
...
| `On Activity, Recalculate` | `Online Charge` | `Online Retry Attempts` |
| `Operand` | `Optional` | `Or Prop Amt` |
| `Or Prop Amt Bnk` | `Orig Contract Date` | `Original Property Amount` |
| `Os Prop Amt` | `Os Prop Amt Bnk` | `Owner` |
| `Parent Account` | `Pay Freq` | `Payment` |
| `Payment Amount` | `Payment Date` | `Payment Freq` |
| `Payment Method` | `Payment Mode` | `Payment Type` |
| `Payment amount` | `Payment type` | `Percentage` |
| `Period` | `Periodic Type` | `Policy.Term` |
| `Pool Sweep` | `Post process` | `Posting Restriction` |
| `Pr Value` | `Pre Notice Activity` | `Pre Notice Days` |
| `Pre processor` | `Prev Settle` | `Principal` |
| `Prioritise` | `Priority Execution` | `Priority Rank Type` |
| `Processing Date` | `Processor` | `Prod Cat End` |
| `Prod Cat Start` | `Product` | `Product Group` |
| `Product Line` | `Product Only` | `Prog Pay Perc` |
| `Property` | `Property 1` | `Property Type` |
| `Property.1` | `Rank Based Priority` | `Rate` |
...
| `Role of the Agent` | `Rule Start` | `Rule Start Date` |
| `Rule.1` | `Schedule Charge` | `Schedule Frequency` |
| `Schedule Name` | `Schedule Type` | `Settle Status` |
| `Settlement A/c` | `Settlement Type` | `Start Date` |
| `Start date` | `StartDt Option` | `Stmt` |
| `Susp Appln` | `Susp End Date` | `Susp Start Date` |
| `Sweep Cancel Date` | `Sys Activity Id` | `Sys App Method` |
| `Sys Auto Settle` | `Sys Charge` | `System Id` |
| `Takeover Npv` | `Takeover method` | `Tax Inclusive` |
| `Term` | `Tier Group` | `Tier Groups` |
| `Tier Type` | `Tot Due Amt` | `Total Due` |
...
| `Value Dtd Acctng Y/N` | `Variation and Pricing details` | `Waive Bill Type` |
| `Waive Class, Waive Prop and Waive Bill Type` | `xx` | `xx` |
| `Field.Value` | `’s` |  |
